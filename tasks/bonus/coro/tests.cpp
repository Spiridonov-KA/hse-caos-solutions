...
TEST_CASE("StackAlignment") {
    uint64_t rsp;
    auto c = Coroutine::Create([&rsp](auto self) {
        rsp = ReadRSP();
        self->Suspend();
        rsp = ReadRSP();
    });
...
TEST_CASE("JustWorks") {
    ...
    {
        auto c = Coroutine::Create([&log](auto self) {
            log.push_back(2);
            self->Suspend();
            log.push_back(4);
        });
...
TEST_CASE("ManyCoroutines") {
    ...
    for (size_t i = 0; i < 1024; ++i) {
        tasks.emplace(Coroutine::Create([i, &log](auto self) {
...
struct FnBody {
    void operator()(auto self) {
        ...
        auto c = Coroutine::Create(FnBody{depth - 1});
        while (!c->IsFinished()) {
            self->Suspend();
            c->Resume();
        }
...
TEST_CASE("NestedCoroutines") {
    auto c = Coroutine::Create(FnBody{128});
    size_t cnt = 0;
    while (!c->IsFinished()) {
        c->Resume();
        ++cnt;
...
TEST_CASE("ExhaustMemory") {
    for (size_t i = 0; i < 65536; ++i) {
        auto c = Coroutine::Create([](auto) {
            volatile char buf[16 * 4096 - 400];
...
TEST_CASE("Performance") {
    ...
    for (size_t i = 0; i < kTasks; ++i) {
        tasks.emplace_back(Coroutine::Create([](auto self) {
